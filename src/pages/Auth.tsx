...
const Auth = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!emailValidation.success) {
      setErrors(prev => ({
        ...prev,
        loginEmail: emailValidation.error.errors[0].message
      }));
      return;
    }
    if (!passwordValidation.success) {
      setErrors(prev => ({
        ...prev,
        loginPassword: passwordValidation.error.errors[0].message
      }));
      return;
    }
    setIsLoading(true);
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!emailValidation.success) {
      setErrors(prev => ({
        ...prev,
        signupEmail: emailValidation.error.errors[0].message
      }));
      return;
    }
    if (!passwordValidation.success) {
      setErrors(prev => ({
        ...prev,
        signupPassword: passwordValidation.error.errors[0].message
      }));
      return;
    }
    if (!nameValidation.success) {
      setErrors(prev => ({
        ...prev,
        signupName: nameValidation.error.errors[0].message
      }));
      return;
    }
    setIsLoading(true);
        ...
        <div className="w-full max-w-md mx-auto lg:mx-0">
          <div className="bg-white dark:bg-card border-2 border-gray-200 dark:border-border rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] p-8 md:p-10">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <Button type="button" variant="outline" className="h-11 bg-white dark:bg-background border-2 border-gray-200 dark:border-border hover:bg-accent hover:text-accent-foreground hover:border-primary/50 transition-all duration-300 group shadow-md hover:shadow-lg" onClick={handleGoogleSignIn}>
              ...
              <Button type="button" variant="outline" className="h-11 bg-white dark:bg-background border-2 border-gray-200 dark:border-border hover:bg-accent hover:text-accent-foreground hover:border-primary/50 transition-all duration-300 group shadow-md hover:shadow-lg">
            ...
            {mode === "login" ? <form onSubmit={handleLogin} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="email" type="email" placeholder="yourmail@email.co.il" value={loginData.email} onChange={e => setLoginData({
                  ...loginData,
                  email: e.target.value
                })} className="h-11 text-right bg-white dark:bg-background border-2 border-gray-200 dark:border-border/60" required />
                  {errors.loginEmail && <p className="text-sm text-destructive text-right">{errors.loginEmail}</p>}
                </div>

                <div className="space-y-2">
                  <Label htmlFor="password" className="text-right block">סיסמה</Label>
                  <div className="relative">
                    <Input id="password" type={showPassword ? "text" : "password"} placeholder="הקלד/י סיסמה" value={loginData.password} onChange={e => setLoginData({
                    ...loginData,
                    password: e.target.value
                  })} className="h-11 text-right pr-10 bg-white dark:bg-background border-2 border-gray-200 dark:border-border/60" required />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors">
                      {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                    </button>
                  </div>
                  {errors.loginPassword && <p className="text-sm text-destructive text-right">{errors.loginPassword}</p>}
                </div>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="signup-name" type="text" placeholder="ישראל ישראלי" value={signupData.fullName} onChange={e => setSignupData({
                  ...signupData,
                  fullName: e.target.value
                })} className="h-11 text-right bg-white dark:bg-background border-2 border-gray-200 dark:border-border/60" required />
                  {errors.signupName && <p className="text-sm text-destructive text-right">{errors.signupName}</p>}
                </div>

                <div className="space-y-2">
                  <Label htmlFor="signup-email" className="text-right block">מייל</Label>
                  <Input id="signup-email" type="email" placeholder="yourmail@email.co.il" value={signupData.email} onChange={e => setSignupData({
                  ...signupData,
                  email: e.target.value
                })} className="h-11 text-right bg-white dark:bg-background border-2 border-gray-200 dark:border-border/60" required />
                  {errors.signupEmail && <p className="text-sm text-destructive text-right">{errors.signupEmail}</p>}
                </div>

                <div className="space-y-2">
                  <Label htmlFor="signup-password" className="text-right block">סיסמה</Label>
                  <div className="relative">
                    <Input id="signup-password" type={showPassword ? "text" : "password"} placeholder="הקלד/י סיסמה" value={signupData.password} onChange={e => setSignupData({
                    ...signupData,
                    password: e.target.value
                  })} className="h-11 text-right pr-10 bg-white dark:bg-background border-2 border-gray-200 dark:border-border/60" required />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors">
                  ...
                  {errors.signupPassword && <p className="text-sm text-destructive text-right">{errors.signupPassword}</p>}
                </div>

                <div className="space-y-3">
                  <Label className="text-right block">אני מתכוון/ת:</Label>
                  <RadioGroup value={signupData.userType} onValueChange={value => setSignupData({
                  ...signupData,
        ...
        <DialogContent className="sm:max-w-md" dir="rtl">
          ...
          <form onSubmit={handleResetPassword} className="space-y-4 mt-4">
            <div className="space-y-2">
              ...
              <Input id="reset-email" type="email" placeholder="yourmail@email.co.il" value={resetEmail} onChange={e => setResetEmail(e.target.value)} className="h-11 text-right" required />
            </div>
