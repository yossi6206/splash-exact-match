    ...
    <DropdownMenu dir="rtl">
      ...
      <DropdownMenuContent align="end" className="w-56 bg-background">
        {shareOptions.map((option) => (
          <DropdownMenuItem
            key={option.name}
            onClick={() => handleShare(option.name, option.link)}
            className="cursor-pointer py-3"
          >
            <div className={`ml-3 ${option.color}`}>{option.icon}</div>
            <span className="text-right flex-1">שתף ב-{option.name}</span>
          </DropdownMenuItem>
        ))}
        <DropdownMenuItem onClick={handleCopyLink} className="cursor-pointer py-3">
          <Share2 className="ml-3 h-5 w-5 text-muted-foreground" />
          <span className="text-right flex-1">העתק קישור</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
